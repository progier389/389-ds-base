...
static void *
ldbm_config_cache_weight_threshold_get(void *arg)
{
    struct ldbm_instance *inst = (struct ldbm_instance *)arg;

    return (void *)((uintptr_t)(inst->cache_weight_threshold));

}

static int
ldbm_config_cache_weight_threshold_set(void *arg,
                               void *value,
                               char *errorbuf,
                               int phase __attribute__((unused)),
                               int apply)
{
    ...
    if (val <= 100) {
        slapi_create_errormsg(errorbuf, SLAPI_DSE_RETURNTEXT_SIZE,
                              "Error: Invalid value for %s (%ld). The value must be greater than \"100\"\n",
                              CONFIG_INSTANCE_CACHE_WEIGHT_THRESHOLD, val);
    ...
    if (apply) {
        inst->cache_weight_threshold = val;
    }
    return LDAP_SUCCESS;
}
...
static config_info ldbm_instance_config[] = {
    ...
    {CONFIG_INSTANCE_CACHE_WEIGHT_THRESHOLD, CONFIG_TYPE_INT, DEFAULT_CACHE_WEIGHT_THRESHOLD_STR, &ldbm_config_cache_weight_threshold_get, &ldbm_config_cache_weight_threshold_set, CONFIG_FLAG_ALLOW_RUNNING_CHANGE},
